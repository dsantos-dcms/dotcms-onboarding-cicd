...
# Using the function
obtained_data = read_yaml_files('./customers/us-east-1/')  # Replace with the correct path
# Consolidate all data into a single dictionary
consolidated_customers_data = {k: v for d in obtained_data for k, v in d.items()}

results = find_new_customers_or_environments(consolidated_customers_data, './config/us-east-1.yaml')
